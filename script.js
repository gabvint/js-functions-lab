// changed some arguments to test if the function is working properly. 
...
const convertTemperature = (temperature, scale) => {
    ...
    if (scale === 'C') { // convert to Fahrenheit
        return (temperature * (9/5)) + 32
    }
    else if (scale === 'F'){ // convert to Celsius 
...
const calculateGrade = (grade) => {
    if (grade >= 90){ // 90 above
        return 'A'; 
    } 
    else if (grade >= 80 && grade < 90){ // 80-89
        return 'B'; 
    }
    else if (grade >= 70 && grade < 80){ // 70-79
        return 'C'; 
    }
    else if (grade >= 60 && grade < 70){ // 60 - 69
        return 'D'; 
    }
    else{
        return 'F'; 
